...
![The dashboard: words written, a streak, and a list to pick back up from](docs/screenshots/dashboard.png)
...
The editor is a sheet of ruled paper rather than a blank rectangle, and it has
its own light and dark independent of the rest of the app — writing at night on
a dark interface but a warm page turns out to be a combination people want.

|                                                                                                                       |                                                                |
| --------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------- |
| ![The editor on paper: ruled lines, a margin rule, and the thought rail alongside](docs/screenshots/editor-paper.png) | ![The same editor at night](docs/screenshots/editor-night.png) |

Thoughts are the unsorted pile. Select several that belong together and forge
them into a single idea — they stay linked, and sit beside you in the editor as
raw material.

![The thoughts page, with several selected for merging](docs/screenshots/thoughts.png)

Ideas carry a status from seed to shipped, and can be archived without being
deleted — the bin empties itself after 30 days, the archive never does.

![The ideas board, filtered by status](docs/screenshots/ideas.png)

Search ranks by relevance rather than date, weighting a title match far above a
passing mention in a draft.

![Search results ranked by relevance](docs/screenshots/search.png)

|                                                                              |                                                                                    |
| ---------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| ![The shipped shelf, listing published pieces](docs/screenshots/shipped.png) | ![Discover: human-written prompts filtered by mood](docs/screenshots/discover.png) |

Screenshots are generated by `npm run screenshots`, so they can be refreshed
when the design moves rather than quietly going stale.
